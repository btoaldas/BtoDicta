...
// aplicaciones de terceros. BtoDicta usa únicamente APIs públicas: un
...
extension Notification.Name {
    static let betoActivacionVozConfiguracionCambio =
        Notification.Name("BtoDictaActivacionVozConfiguracionCambio")
    static let betoActivacionVozEstadoCambio =
        Notification.Name("BtoDictaActivacionVozEstadoCambio")
}
...
final class ActivacionVoz: @unchecked Sendable {
    ...
    struct Despertar {
        ...
        enum Origen: String {
            ...
            /// BtoDicta ya poseía el micrófono y reconoció localmente la frase
    ...
    enum Estado: Equatable {
        ...
        var descripcion: String {
            ...
            case .pausado: return "En pausa mientras BtoDicta usa el micrófono o habla"
            case .sinFrases: return "Agrega al menos una frase segura de dos palabras"
            case .sinPermiso: return "Activa el acceso al micrófono para BtoDicta en Privacidad y seguridad"
    ...
    private let cola = DispatchQueue(label: "btodicta.activacion-voz", qos: .utility)
    ...
    private static var debug: Bool {
        ProcessInfo.processInfo.environment["BTODICTA_WAKELIVETEST"] == "1"
    }
    ...
    static func ejecutarQA() -> (Bool, [String]) {
        ...
        let s = caso("pasarela Siri rechaza ruta, token y credenciales ajenas",
                     !PasarelaSiriBeto.esOrdenEscuchar(
                        URL(string: "btodicta://agente/borrar?t=\(tokenQA)")!,
                        tokenEsperado: tokenQA)
                        && !PasarelaSiriBeto.esOrdenEscuchar(
                            URL(string: "btodicta://usuario:clave@agente/escuchar?t=\(tokenQA)")!,
                            tokenEsperado: tokenQA)
                        && !PasarelaSiriBeto.esOrdenEscuchar(
                            URL(string: "btodicta://agente/escuchar")!,
                            tokenEsperado: tokenQA)
                        && !PasarelaSiriBeto.esOrdenEscuchar(
                            URL(string: "btodicta://agente/escuchar?t=incorrecto")!,
                            tokenEsperado: tokenQA))
