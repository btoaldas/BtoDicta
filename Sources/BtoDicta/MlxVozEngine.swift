...
//
// El runtime vive aislado en ~/.btodicta/voz-engine/mlx-venv. El modelo se baja una
...
enum MlxVozEngine {
    ...
    /// Por ahora admitimos únicamente los dos modelos que BtoDicta presenta en su GUI.
...
enum MlxVozServer {
    ...
    static func asegurar(voz: VozLocal, protegerMinutos: Double = 0,
        ...
        if let s = saludActual(), s.motor == "btodicta-qwen-mlx" {
            if s.identidad == id {
    ...
    private static func pcmFloatAWav(_ f32: Data) throws -> Data {
        ...
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("btodicta-mlx-\(UUID().uuidString).wav")
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith("/health"):
            body = json.dumps({"motor":"btodicta-qwen-mlx", "identidad":IDENTIDAD, "pid":os.getpid()}).encode()
...
private final class MlxStreamPlayer: NSObject, URLSessionDataDelegate {
    ...
    private let callbacks: OperationQueue = {
        let q = OperationQueue(); q.name = "btodicta.mlx-stream"; q.maxConcurrentOperationCount = 1; return q
    }()
