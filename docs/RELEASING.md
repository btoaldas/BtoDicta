# Cómo se publica un release de BtoDicta (gobernanza)
...
5. **Publicar** — `gh release create` con el DMG **versionado** y el DMG **estable** `BtoDicta.dmg` (el que consume el tap de Homebrew: `releases/latest/download/BtoDicta.dmg`).
...
Antes de correrlo: subir la versión en `Sources/BtoDicta/Version.swift` (número, fecha y una entrada en `historial`) y en `Info.plist` (`CFBundleShortVersionString` y `CFBundleVersion`).
...
`BTODICTA_RELEASE_DIR=build/releases/v<VERSION>-intento2`. Nunca se limpian
...
`BTODICTA_DMG_HEADLESS=1` permite generar el DMG sin automatizar Finder. Antes
